...
def aprioriImpl(part):
    ...
    while(len(interim_list) > 0):
        ...
        if k == 2:
            ...
            #print('Length of interim_list-->', len(interim_list))

            #Generating pairs for k = 1 candidates
            pairs = [set(i) for i in (itertools.combinations(sorted(interim_list), k))]

            #print('Length of Generated pairs', len(pairs))
            ...
            #print('Candidates for which count will be stored --> ', pairs)
        ...
        k += 1
        #print('value for k =',k)
        #print('for k=',k,' counts of elements',count_pair)
        #filtering pairs for threshold


        '''
        for key in count_pair.keys():
            if count_pair[key] >= red_support:
                interim_list.append(key)
                candidate_list.append(key)'''

    #print('Candidate list for respective partition --> ',candidate_list)
...
#basket creation
bsk_rdd = hdr_rem_rdd.groupByKey().map(lambda x: (x[0],set(x[1]))).filter(lambda x:len(x[1])>k).persist()
print(bsk_rdd.count())
#sys.exit()
...
a = bsk_rdd.mapPartitions(secPhase).groupByKey().map(lambda x:(x[0],sum(x[1]))).filter(lambda x:x[1]>=support).map(lambda x:x[0]).collect()
